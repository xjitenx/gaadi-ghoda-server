    ...
    public class UserService
    {
        public User getUserProfile(Guid userId)
        {
            ...
            using (var connection = new NpgsqlConnection(AppConstants.CONNECTION_STRING))
            {
                ...
                {
                    //use command here
                    command.CommandText = "select org_id, user_id, user_alias, user_first_name, user_last_name, user_email_id, user_contact_no from public.tbl_user where user_id = @userId";
                    ...
                    using (NpgsqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            userProfile.Alias = reader.GetString(reader.GetOrdinal("user_alias"));
